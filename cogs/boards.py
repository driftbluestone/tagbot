...
class Boards(commands.Cog):
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.Member):
        ...
        if emoji not in server_config["boards"].keys(): return
        if reaction.count != server_config["boards"][emoji][1]: return
        channel = await self.bot.fetch_channel(server_config["boards"][emoji][0])
